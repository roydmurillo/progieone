...
jQuery('body').on('click','#next',function(){
	var data_obj = {start:parseInt(jQuery('#paginate_page').val()) + 1, show_entry:jQuery("#show_entry").val()};
...
jQuery('body').on('change','#paginate_page',function(){
	var data_obj = {start:jQuery(this).val(), show_entry:jQuery("#show_entry").val()};
	data_obj = jQuery.toJSON(data_obj);	
	loader2();
	jQuery.ajax({
			type: 'POST',
