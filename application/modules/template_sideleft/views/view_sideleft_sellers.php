...
<?php }	?>
  ...
  <input type="hidden" value="<?php echo trim($refine); ?>" id="refine">
    <input type="hidden" value="<?php echo trim($refine_rating); ?>" id="refine_rating">
  <input type="hidden" value="<?php echo current_url(); ?>" id="current_url">
   <?php }?>
  <?php
  	$get = array();
...
<?php $this->load->module("function_security"); 
	  $ajax = $this->function_security->encode("load_refine_search_sellers"); ?>
...
jQuery(document).ready(function(){
		...
		jQuery.ajax({
            ...
            cache: false,
            data: { args: jQuery("#refine").val(), refine_rating: jQuery("#refine_rating").val(), url:jQuery("#current_url").val(), get:jQuery("#get_values").val(), uri_process:jQuery("#uri_process").val() }
